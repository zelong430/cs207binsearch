...
def binary_search(da_array: list, needle, left:int=0, right:int=-1) -> int:
    ...
    PRE: `da_array` is sorted in non-decreasing order (thus items in
        ...
        If 'da_array' is not sorted, the result is not guaranteed and
        may be anything.
    POST: 
    ...
    if len(da_array) == 0:
        return -1
    if not isinstance(da_array, collections.Sequence):
        raise TypeError("da_array isn't a sequence")
    if needle > np.inf+1 or needle<-np.inf-1:
        return -1
    if np.nan in da_array:
        raise TypeError('Nan detected')
